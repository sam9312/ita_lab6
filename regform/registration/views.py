...
from webapp.models import ScotlandYard
...
@login_required(login_url='/admin/login/')
def scotlandyard(request):
    
    found_entries = None
    found_entries = ScotlandYard.objects.all().order_by("id")
                    
    return render(request, 'useradmin/scotlandyard.html', {'results': found_entries, })
